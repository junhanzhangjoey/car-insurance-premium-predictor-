DECLARE input_make  STRING DEFAULT @make;
DECLARE input_model STRING DEFAULT @model;
DECLARE input_year  INT64  DEFAULT @year;
...
target AS (
  SELECT adjusted_risk AS t_risk
...
stats AS (
  -- note: COUNT/COUNTIF without GROUP BY always returns one row,
  -- even if COUNT(*) = 0
  SELECT
...
SELECT
  ROUND(
    SAFE_DIVIDE(le_count, total_count) * 30
