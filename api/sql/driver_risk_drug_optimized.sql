DECLARE input_state STRING DEFAULT @state;
DECLARE input_age   INT64  DEFAULT @age;
DECLARE input_drug STRING DEFAULT @drug;
WITH drug_fatalities AS (
  ...
  WHERE p.PER_TYP = "Driver of a Motor Vehicle in Transport" AND p.STATE = input_state
    AND d.DRUGRES IN (
      "Depressant", 
      "Other Drug", 
      "Stimulant", 
